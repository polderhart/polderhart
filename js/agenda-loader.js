...
(function () {
  ...
  async function loadEvents() {
    ...
    if (events.length === 0) {
      container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: var(--space-3xl) 0;">Geen evenementen gepland.</p>';
      return;
    }
    ...
    Object.keys(grouped).sort().forEach(function (schoolYear) {
      const [startYear, endYear] = schoolYear.split('-');
      html += '<div class="agenda-year"><h2 class="agenda-year__title">Schooljaar ' + startYear + '-' + endYear + '</h2></div>';

      const months = grouped[schoolYear];
      Object.keys(months).sort().forEach(function (monthKey) {
        const monthData = months[monthKey];
        html += '<div class="agenda-month"><h3 class="agenda-month__title">' + monthData.label + '</h3>';
        ...
        monthData.events.forEach(function (event) {
          ...
          if (event.time) {
            html += '<p class="agenda-event__time">' + escapeHtml(event.time) + '</p>';
          }
          if (event.description) {
            html += '<p class="agenda-event__description">' + escapeHtml(event.description) + '</p>';
          }
          html += '</div></div>';
        });
