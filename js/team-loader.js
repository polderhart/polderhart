...
class TeamLoader {
      ...
      if (data.group_photo) {
        ...
        if (img && section) {
          ...
          img.src = onNetlify
            ...
            : data.group_photo;
          img.alt =
            data.group_photo_alt || "Het volledige team van GBS 't Polderhart";
          if (data.group_photo_position)
            img.style.objectPosition = data.group_photo_position;
  ...
  async init() {
    ...
    if (!this.container) {
      console.warn(
        "Team container niet gevonden. Zorg dat #team-container bestaat in team.html",
      );
      return;
    }
  ...
  renderMemberCard(member) {
    ...
    const photoSrc = hasPhoto
      ? onNetlify
        ? `/.netlify/images?url=${encodeURIComponent(member.photo)}&w=400&q=80`
        : member.photo
      : "";
    const imgStyles = [];
    if (member.photo_position)
      imgStyles.push(`object-position:${member.photo_position}`);
    if (member.photo_zoom && member.photo_zoom !== "1")
      imgStyles.push(`--photo-zoom:${member.photo_zoom}`);
    const positionStyle = imgStyles.length
  ...
  render() {
    ...
    if (
      leerkrachten.subsections.kleuterklassen.members.length > 0 ||
    ...
    if (
      ondersteunend.subsections.admin.members.length > 0 ||
